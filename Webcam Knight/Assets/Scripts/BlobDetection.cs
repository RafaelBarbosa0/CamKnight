...
namespace CamKnight
{
    public class BlobDetection : MonoBehaviour
    {
        ...
        [SerializeField]
        private float adjustPosX;
        [SerializeField]
        private float adjustPosY;

        public Blob MainTipBlob { get => mainTipBlob; private set => mainTipBlob = value; }
        public Blob MainBaseBlob { get => mainBaseBlob; private set => mainBaseBlob = value; }

        private void Start()
        {
            // Set pixel colors.
            tipPixelColor = Color.green;
            basePixelColor = Color.red;
        ...
        private void Update()
        {
            ...
            else
            {
                ...
                if (Input.GetKeyDown("f"))
                {
                    ...
                    foreach(System.Numerics.Vector2 point in mainTipBlob.PointList)
                    {
              
                        Vector3 worldPos = Camera.main.ScreenToWorldPoint(new Vector3(Screen.width - point.X, point.Y, -1));
                        GameObject cube = GameObject.CreatePrimitive(PrimitiveType.Cube);

                        cube.transform.position = new Vector3(worldPos.x - adjustPosX, worldPos.y - adjustPosY, -1);
                    }



                    Debug.Log(tipBlobs.Count);
                    Debug.Log(baseBlobs.Count);
                }
            }
        }
                    ...
                    else
                    {
                        ...
                        foreach (Blob blob in blobs)
                        {
                            if (IsPartOfBlob(blob, thisPointsSystem))
                            {
                                ...
                                blob.AddToPointList((int)thisPointsSystem.X, (int)thisPointsSystem.Y);
                                partOfBlob = true;
                                break;
                            }
                        }
        ...
        private Blob GetBiggestBlog(List<Blob> blobs)
        {
            ...
            foreach (Blob blob in blobs)
            {
                if(blob.PointList.Count > biggest)
                {
                    selectedBlob = blob;
